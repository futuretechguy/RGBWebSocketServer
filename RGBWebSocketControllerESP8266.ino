...
int Purple[3]    = { 64, 0, 255 };
int Treegreen[3]  = { 0, 100, 0 };
int Deepblue[3]   = { 0, 0, 100 };
...
void handleFileList() {
  ...
  while (dir.next()) {
    ...
    json = getjsonfilelist("(isDir)? dir, file", String(entry.name()).substring(1));
    entry.close();
  }
...
void ProcessRGBReguest(String rgbcmd) {
  String json;
...
void setup(void) {
  ...
  server.on("/rgbm.html", []() {                        //load RGB control page
  ...
  server.onNotFound([]() {                               //called when the url is not defined here
    ...
    if (isRGBrequest(server.uri())) {
      ProcessRGBrequest(server.uri());  //process RGB GET commands

    } else if (!handleFileRead(server.uri())) {
      server.send(404, "text/plain", "CommandNotFound");    //use it to load content from SPIFFS
...
void ProcessRGBrequest (String message) {
